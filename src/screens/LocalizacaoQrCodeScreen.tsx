...
export default function LocalizacaoQrCodeScreen() {
  ...
  const { data, localizacao } = route.params as { 
    data: string; 
    localizacao: any 
  };
  ...
  const handleShare = async () => {
    try {
      const message = `Localização: ${localizacao.armazem}\nRua: ${localizacao.rua}\nMódulo: ${localizacao.modulo}\nCompartimento: ${localizacao.compartimento}`;
      
      await Share.share({
        message: message,
        title: `QR Code Localização ${localizacao.armazem}`
      });
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>QR CODE DA LOCALIZAÇÃO</Text>
      ...
      <View style={styles.infoContainer}>
        <Text style={styles.infoText}>Armazém: {localizacao.armazem}</Text>
        <Text style={styles.infoText}>Rua: {localizacao.rua}</Text>
        <Text style={styles.infoText}>Módulo: {localizacao.modulo}</Text>
        <Text style={styles.infoText}>Compartimento: {localizacao.compartimento}</Text>
      </View>
